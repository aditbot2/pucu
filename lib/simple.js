const { default: makeWASocket, makeWALegacySocket, extractMessageContent, makeInMemoryStore, proto, prepareWAMessageMedia, downloadContentFromMessage, getBinaryNodeChild, jidDecode, areJidsSameUser, generateWAMessage, generateForwardMessageContent, generateWAMessageFromContent, getContentType, WAMessageStubType, WA_DEFAULT_EPHEMERAL } = require('@whiskeysockets/baileys')
...
exports.makeWASocket = (connectionOptions, options = {}) => {
  ...
  conn.sendMessageDims = (jid, teks = '', quoted = '', opts = {}) => {
    return conn.sendMessage(jid, {
        ...
        contextInfo: {
            ...
            externalAdReply: {
                ...
                title: opts.title ?? global.set.wm,
                body: opts.body,
                mediaType: 1,
                previewType: 'PHOTO',
    ...
    let location = {
      ...
      message: {
        "imageMessage": {
          "mimetype": "image/jpeg",
          "caption": caption,
          "jpegThumbnail": fs.readFileSync(`./src/mari.jpg`)
        }
      }
    }
  ...
  conn.sendContact = async (jid, contact, quoted, info = {}, opts = {}) => {
    ...
    contact.map(v => list.push({
      displayName: v.name,
      vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:${v.name}\nORG:${info && info.org ? info.org : 'Im-Dims'}\nTEL;type=CELL;type=VOICE;waid=${v.number}:${PhoneNumber('+' + v.number).getNumber('international')}\nEMAIL;type=Email:${info && info.email ? info.email : 'Ipungrasta995@gmail.com'}\nURL;type=Website:${info && info.website ? info.website : 'https://github.com/Im-Dims'}\nADR;type=Location:;;${info && info.location ? info.location : 'Jepang'};;\nOther:${v.about}\nEND:VCARD`
    }))
    return conn.sendMessage(jid, {
      contacts: {
  ...
  /**
  * Footer By Im-Dims
  * Don't remove this wm!
  */
  conn.sendFooter = async (jid, text, footer, quoted, options = {}) => {
    await conn.sendPresenceUpdate('composing', jid)
  ...
  conn.sendFooterWithMedia = async (jid, text, footer, media, quoted, options = {}) => {
    await conn.sendPresenceUpdate('composing', jid)
    let msg = await generateWAMessageFromContent(jid, {     
      interactiveMessage: {          
        body: {         
  ...
  conn.sendFooterWithVideo = async (jid, text, footer, media, quoted, options = {}) => {
    await conn.sendPresenceUpdate('composing', jid)
    let msg = await generateWAMessageFromContent(jid, {     
      interactiveMessage: {          
        body: {         
        ...
        header: {
          hasMediaAttachment: false,
          ...await prepareWAMessageMedia({ video: { url: media } }, { upload: conn.waUploadToServer })
        },    
        nativeFlowMessage: {
          buttons: [{ title: "Dims ganteng >,<" }]
        }   
  ...
  conn.sendFooterWithVideoPath = async (jid, text, footer, media, quoted, options = {}) => {
    await conn.sendPresenceUpdate('composing', jid)
    let msg = await generateWAMessageFromContent(jid, {     
      interactiveMessage: {          
        body: {         
        ...
        header: {
          hasMediaAttachment: false,
          ...await prepareWAMessageMedia({ video: media }, { upload: conn.waUploadToServer })
        },    
        nativeFlowMessage: {
          buttons: [{ title: "Dims ganteng >,<" }]
        }   
  ...
  conn.sendFooterWithMediaPath = async (jid, text, footer, media, quoted, options = {}) => {
    await conn.sendPresenceUpdate('composing', jid)
    let msg = await generateWAMessageFromContent(jid, {     
      interactiveMessage: {          
        body: {         
        ...
        header: {
          hasMediaAttachment: false,
          ...await prepareWAMessageMedia({ image: media }, { upload: conn.waUploadToServer })
        },    
        nativeFlowMessage: {
          buttons: [{ title: "Dims ganteng >,<" }]
        }   
...
conn.sendReplygc = async (jid, captionini, mentiot, titleaa, bodyaa, thumbwoy, sourceurlll, options = {}) => {
await conn.sendMessage(jid, { text: captionini, contextInfo: { mentionedJid: mentiot, forwardingScore: 9999999, isForwarded: true, "externalAdReply": { "showAdAttribution": true, "containsAutoReply": true, "title": titleaa, "body": bodyaa, "previewType": "PHOTO", "thumbnail": thumbwoy, "sourceUrl": sourceurlll }}}, { quoted: options })
}

  conn.sendMusicWithUrl = async (jid, title, body, thumb, source, caption, quoted, options = {}) => {
        let contextInfo = {
            externalAdReply: {
    ...
    return await conn.sendMessage(jid, { contextInfo, mimetype: 'image/jpeg', text: caption }, { quoted, ...options })
  };
   
  conn.sendMusicWithPath = async (jid, title, body, thumb, source, caption, quoted, options = {}) => {
        let contextInfo = {
            externalAdReply: {
               showAdAttribution: true, 
                  title: title,
